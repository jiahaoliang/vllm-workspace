...
Blockwise DSA PD offload 沿用穿刺 connector 的 TP coverage：要求 `P_TP % D_TP == 0`，将 Prefill TP 连续分组，并由每组首个 rank 作为对应 Decode TP 的唯一 payload source。Indexer、可选 Indexer scale、Main K 和 Main V 共用该 leader；其他组内 rank 不发送 payload。这个选择避免首版实现多 P shard 拼装和多 writer destination，但要求 leader 持有完整 replica，并比普通 `MooncakeConnectorV1` 增加更严格的整除约束。
