...
record_step "namespace safety gate" "${output_dir}/namespace.txt" \
  kubectl get namespace -n "${namespace}" "${namespace}" -o name \
...
identity_check() {
  ...
  test "$(git -C repos/vllm-ascend rev-parse HEAD)" = 45b2e785b10ca4604cd6314819ed15f3ff674781
  test "$(git -C repos/Mooncake rev-parse HEAD)" = df3f74ed8ebdb0c935554beea6299a9f11c723e2
...
for revision in 54503ecec0f3ac31e5ecfc5f28652e4cc42307b5 45b2e785b10ca4604cd6314819ed15f3ff674781 df3f74ed8ebdb0c935554beea6299a9f11c723e2; do
...
record_step "verify native image source" "${output_dir}/source-sync.log" "${script_dir}/sync-vllm-ascend-python.sh" || { fail_run "native source identity failed"; exit 1; }
for role in prefill decode; do
  pod_variable=${role}_pod
  pod=${!pod_variable}
...
{
  git -C "${workspace_root}/repos/vllm-ascend" diff --name-only --diff-filter=ACMRT 45b2e785b10ca4604cd6314819ed15f3ff674781 -- vllm_ascend
