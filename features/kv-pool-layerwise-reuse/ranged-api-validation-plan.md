...
当前 G3 fixture 基于已 push 的 concurrent cache isolation smoke，并由
`test(kv_pool): tighten deployment smoke acceptance` commit 收紧 HTTP 200 和 fallback serial
replay 判定。它包含 runtime ConfigMap、runbook、smoke wrapper、validation record 和 status，
是本轮默认 authoritative fixture。
...
T0 必须确认实际执行文件仍与 `2d0bd8a` 一致。如果这些文件之后再次出现修改，则当前
...
- [ ] 确认 G3 fixture commit `2d0bd8a` 可 fetch，且相关 deployment files 与该 commit 一致。
...
`os.getenv`。默认值为 `0`，仅接受 `0/1`，并标记为非敏感配置。

事件使用固定前缀加单行 JSON，checker 只解析该前缀，不能依赖自由文本日志：

```text
[KVPOOL_RANGE_DEBUG] {"event":"range","direction":"save","layer_id":3,
  "key_count":2,"requested_bytes":[4096,4096],"sizes":[[2048,2048],[4096]],
  "object_offsets":[[12288,14336],[12288]],"results":[4096,4096]}
[KVPOOL_RANGE_DEBUG] {"event":"commit","layer_id":26,"key_count":2,
  "results":[0,0]}
[KVPOOL_RANGE_DEBUG] {"event":"whole_key","direction":"put","key_count":2}
```

以上为 schema 示例，JSON 必须保持单行；文档换行只为可读性。字段契约如下：

- `range`：`direction=save|load`、physical `layer_id`、`key_count`、nested `sizes`、nested
  `object_offsets`、每个 key 的 `requested_bytes` 和原始 `results`；
- `commit`：final physical layer ID、committed key count 和原始 `batch_commit` results；
- `whole_key`：`direction=put|get` 和 key count，仅作为 backstop path 计数；
- 不记录 key 内容、request ID、pointer、GVA、prompt 或生成文本；
- disabled path 必须在构造 `requested_bytes`、JSON payload 或复制 nested list 之前退出；
- instrumentation 自身的序列化/logger exception 必须被隔离，不能改变原 API result、failure
  policy、cleanup 或推理行为。

准确 call site：

- save `range`：`kv_transfer.py` 的 range-save `_handle_range_request()` 中，
  `batch_copy_put()` 返回并通过 `require_aligned_batch_results()` 后；
- load `range`：range-load `_handle_range_request()` 中，`batch_copy_get()` 返回并通过
  `require_aligned_batch_results()` 后；
- `commit`：final-layer `batch_commit()` 返回并通过 result-shape check 后；
- `whole_key`：`mooncake_backend.py` 的 legacy `put()`/`get()` 入口，调用
  `batch_put_from_multi_buffers()`/`batch_get_into_multi_buffers()` 之前；
- backend 的 `batch_copy_put()`/`batch_copy_get()` 不记录 `whole_key`，避免把 ranged wrapper
  误分类为 backstop path。

layer ID 只存在于 orchestration 层，因此 ranged event 放在 `kv_transfer.py`，不能下沉到没有
physical-layer context 的 Mooncake backend wrapper。

### 11.4 Focused tests 和 checker

只在现有 test files 增加 focused cases：

- `tests/ut/test_envs.py`：默认关闭、`0`、`1` 和非法值行为；
- `test_kv_transfer.py`：enabled save/load event 字段、nested byte sum、physical layer ID、final
  commit event 和日志顺序；
- `test_kv_transfer.py`：disabled path 不调用 payload builder，mock serializer/logger 抛异常时
  原 ranged result 和 failure propagation 不变；
- `test_backend.py`：legacy whole-key `put/get` 各产生一个 backstop event，ranged wrapper 不产生；
- existing malformed/partial failure、revoke、get-end 和 shared-owner tests 保持原断言，不复制成
  log-based state tests。

control repo 最多新增：

```text
features/kv-pool-layerwise-reuse/deployment/check-range-debug-log.py
```

checker 读取一次 clean request 的 prefill/decode logs 和显式 `--num-layers`。`num_layers` 必须从
本轮模型 `config.json` 的 `num_hidden_layers` 读取并进入 artifact，不能在 checker 中硬编码。
checker 使用 JSON parser 解析事件，并断言：
...
owner、shared-key、get-end、并发隔离和输出正确性继续由 G2/G3 负责，不在 runtime log 重建。
G4 只证明这一条 clean production request 的 physical-layer ranged path。

### 11.5 Source 和 runtime workflow

进入 T5 前按 workspace 规则重新确认 vLLM-Ascend branch、remote、locked commit 和 clean
worktree。源码阶段：

1. 修改严格限制在 §11.2 文件和一个 control-repo checker；
2. 运行三个 affected test files 和完整 AscendStore targeted UT；
3. 运行 `bash format.sh ci`，review 自动修复 diff，禁止保留无关格式修改；
4. 创建 signed vLLM-Ascend source commit 并 push personal fork；
5. Python-only sync 到两个现有 Pod，不替换 Pod；
6. 刷新 `workspace.lock.json`、`repo-state.md` 和 `sync-log.md`。

runtime audit 必须使用独立日志窗口，且测试期间不能有其他 inference traffic：

1. 停止两个 vLLM，重启 Mooncake Master，确认 `master_key_count=0`；
2. 用模型 `config.json` 读取并记录 `num_hidden_layers`；
3. 使用以下命令分别启动两个 engine；现有 start scripts 会 truncate engine logs，环境变量会被
   其启动的 Python 子进程继承：
...
4. 等待两个 `/v1/models` 实际返回 HTTP 200，不能只依赖可能仍为 true 的 Kubernetes Ready；
5. 通过 standard proxy 发送一个 deterministic long-prompt completion request。prompt 必须超过
   一个 KV block 但不超过 `max_model_len`，使用 `temperature=0`、固定 seed、非 streaming，并将
   完整 request/response 保存为 artifact；不新增 request-generator framework；
6. 确认 response HTTP 200、choices 非空和 decoder KV hit 后，立即收集两个 engine logs；
7. 运行 checker，保存 machine-readable summary，再生成和验证独立 `SHA256SUMS`。

G4 使用新的 `/tmp/ranged-api-g4-<UTC timestamp>` staging 和独立 workspace-external 持久归档，
不能修改已经完成 checksum 的 G0-G3 artifact。至少保存：
...
runtime-audit/
  ...
  vllm-prefill.log
  vllm-decode.log
  range-debug-summary.json
  SHA256SUMS
```

任一 layer 缺失、byte mismatch、commit ordering 错误、whole-key event、logger 影响行为、非 200
response 或 checksum failure 都使 G4 failed；不能用 G1-G3 passed 覆盖 G4 failure。
...
- [ ] clean-log-window single request 覆盖全部 save/load physical layers，whole-key event 为 0。
- [ ] 每个 ranged result 与对应 fragment byte sum 一致，final commit ordering 正确。
- [ ] checker machine-readable summary 为 passed，独立 G4 artifact checksum 在持久目录复验通过。
