...
| Test 2 performance | BULK | 1 | 40 | 8 | default 0, not passed |
| Test 2 performance | REUSE3 | 1 | 40 | 8 | default 0, not passed |
| Test 2 capacity | BULK | 1 | 40 | 40 | default 0, not passed |
| Test 2 capacity | REUSE3 | 1 | 40 | 40 | default 0, not passed |
...
partial-prefill option is passed.
...
Test 1 additionally passes `--long-prefill-token-threshold 1024`. Test 2 does
not pass that option. The Prefill `KV_JSON` values are:
...
`formal-1.jsonl` to a fresh run directory, generate config, then execute:
...
1, 100 for Test 2). Stop a capacity point at admission if the service OOMs or
times out; do not change one variant's parameters.
...
| Test 2 performance | BULK | admission OOM | N/A | 8 running, 31 waiting; 158 MiB allocation failed |
| Test 2 performance | REUSE3 | admission OOM | N/A | 8 running, 9 waiting; 158 MiB allocation failed |
| Test 2 capacity | BULK | admission OOM | N/A | at most 6 observed running; 146 MiB allocation failed |
| Test 2 capacity | REUSE3 | admission OOM | N/A | at most 5 observed running; 66/256 MiB allocations failed |

`LAYERWISE/BULK = 0.3589 / 0.2721 = 1.3190`. Test 2 has no valid performance
ratio: neither variant passed admission with threshold left at its default 0.
The capacity group also does not prove 40 resident contexts for REUSE3; both
variants failed before formal traffic, and those failures are the capacity
boundary result rather than an invitation to tune variants differently.
