...
**技术栈：** Python、vLLM v0.23.0、vLLM Ascend、Mooncake Python Client、
...
- 使用 `repos/vllm` 的 `v0.23.0` tag（commit
  `0fc695fc6d1d82e9a5ac6835ac8e4e1c83703665`）进行验证。
...
- 保持 `repos/Mooncake` 只读；在 contract gate 和 NPU gate 阶段集成 Mooncake
  团队提供的 wheel/commit。
- Mooncake block-key layerwise 的首期支持范围为基于 TP 的 MLA 和 GQA。当 PP、
  PCP 或 DCP world size 大于 1 时，拒绝启用 Mooncake layerwise。
...
**问题：** backport 到旧 v0.20.2rc1 分支、使用当前 b792 分支搭配 vLLM v0.23.0，
还是使用其已验证的 main commit？

**决策：** 从 `b792c37` 实施，并基于 vLLM v0.23.0 release tag 验证。vLLM 源码
保持不变。
...
**决策：** 仅支持基于 TP 的 MLA/GQA。当 PP、PCP 或 DCP 大于 1 时，Mooncake
block-key layerwise 必须快速失败，以防
...
|---|---|---|---|
| Task 0 基线 | `implementation baseline` | `Implement the Mooncake layerwise Backend contract.` | 记录 vLLM v0.23.0 SHA，以及新的 vLLM Ascend branch/base SHA。 |
...
- 输出：vLLM v0.23.0 checkout 和个人 vLLM Ascend feature branch。
...
git -C repos/vllm switch --detach v0.23.0
git -C repos/vllm rev-parse HEAD
```

预期：vLLM 报告 `0fc695fc6d1d82e9a5ac6835ac8e4e1c83703665`；vLLM
...
预期：lock/repo-state 记录 vLLM v0.23.0 和
...
要求，在 `sync-log.md` 追加带日期的条目，其中包含准确的 vLLM v0.23.0 SHA、分支名和
`$sourceCommit`。
...
当 `backend_name == "mooncake"` 且启用 block-key layerwise 时，若 PP、PCP 或 DCP
size 大于 1，则拒绝启动；错误消息应列出不支持的维度及其配置值。不要将此新检查应用于
旧 Mooncake whole-key 模式。
