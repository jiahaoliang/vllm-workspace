...
- lease expiry must invalidate only that get session, so a fresh
  `batch_get_start` can acquire a new lease while the committed object remains;
...
1. `batch_put_start(key, 8192)` must return `[0]`.
...
5. `batch_put_end(key)` must return `[0]`.

There is no `batch_get_start` before `batch_put_end`. No read lease exists
...
1. After commit, call `batch_get_start(key)` once; it must return `[0]`.
...
1. Call a fresh `batch_get_start(key)`; it must return `[0]`.
2. Read layer 1 from offset `4096`; result must be `[4096]`.
3. `batch_get_end(key)` must return `0`.
