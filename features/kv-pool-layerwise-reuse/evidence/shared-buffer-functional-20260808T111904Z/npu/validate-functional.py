...
def validate_reuse_loads(log: str, warm: bool) -> dict[str, object]:
    matching = [
        event
        for event in events(log)
        if event.get("event") == "range" and event.get("direction") == "load"
    ]
    expected_group_count = 31 if warm else 15
    assert len(matching) == expected_group_count * len(EXPECTED_LAYERS), len(matching)
    key_counts: Counter[int] = Counter()
    for group_index in range(expected_group_count):
        start = group_index * len(EXPECTED_LAYERS)
        group = matching[start : start + len(EXPECTED_LAYERS)]
        assert [int(event["layer_id"]) for event in group] == EXPECTED_LAYERS
        warm_prefix_group = warm and group_index == 15
        for event in group:
            layer_id = int(event["layer_id"])
            key_count = int(event["key_count"])
            expected_key_count = (
                4 if warm_prefix_group else 1 if layer_id in INDEPENDENT_LAYERS else 5
            )
    ...
    return {
        ...
        "independent_decode_key_count": 1,
        "shared_decode_key_count": 5,
        "warm_prefix_key_count": 4 if warm else None,
    }
...
def main() -> int:
    ...
    for name, case_dir, config, request_count, expected_master_keys in (
        ...
        if config is None:
            ...
            load = validate_reuse_loads(log, warm=name == "both-reuse")
    ...
    result = {
        ...
        "status": "passed",
        "validated": True,
        "run_id": os.environ.get("FUNCTIONAL_RUN_ID", "20260808T111904Z"),
