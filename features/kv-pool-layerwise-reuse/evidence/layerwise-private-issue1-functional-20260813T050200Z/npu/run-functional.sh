...
readonly prefill_label=app=layerwise-issue1-functional-prefill
readonly master_deployment=layerwise-issue1-functional-master-20260813t050200z
readonly expected_image=${EXPECTED_IMAGE:-docker.io/library/vllm-ascend:layerwise-issue1-8653c6c5e-20260813T045514Z}
readonly expected_manifest=${EXPECTED_MANIFEST:-sha256:8c9f462c2c59e5c3ca4f6d7e89691ac20ae0f94943c470a7b57f2b9590ea6495}
readonly expected_config=${EXPECTED_CONFIG:-sha256:f7aec20a062b9fcc18892b8639c707d9d89b7a29954aa2e23f31d77b0ab88de7}
readonly asset_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly script_dir=${OUTPUT_DIR:-${asset_dir}}
readonly remote_root=${REMOTE_ROOT:-/tmp/layerwise-private-issue1-functional-20260813T050200Z}
...
resolve_prefill_pod() {
  kubectl get pods -n "${namespace}" -l "${prefill_label}" -o json | jq -r '
...
while time.monotonic() < deadline:
    try:
        print(urlopen("http://layerwise-issue1-functional-master-20260813t050200z:9003/metrics", timeout=5).read().decode(), end="")
        raise SystemExit(0)
    except Exception as error:
        last_error = error
    time.sleep(0.5)
...
last = None
while time.monotonic() < deadline:
    text = urlopen("http://layerwise-issue1-functional-master-20260813t050200z:9003/metrics", timeout=5).read().decode()
...
restart_master() {
  kubectl rollout restart -n "${namespace}" "deployment/${master_deployment}"
  kubectl rollout status -n "${namespace}" "deployment/${master_deployment}" --timeout=300s
}
...
failure_cleanup() {
  ...
  if (( rc != 0 && cleanup_complete == 0 )); then
    ...
    kubectl rollout status -n "${namespace}" "deployment/${master_deployment}" \
...
record "assert Prefill identity" "${script_dir}/prefill-identity-assert.log" jq -e \
  ...
  --arg manifest "${expected_manifest}" '
    ...
    .status.phase == "Running" and .spec.nodeName == "m1" and
    .spec.containers[0].image == $image and
    (.status.containerStatuses[0].imageID | endswith($config) or endswith($manifest)) and
...
record "capture embedded source identity" "${script_dir}/embedded-source-identity.txt" \
  kubectl exec -n "${namespace}" "${prefill_pod}" -c prefill-engine -- bash -c \
  'set -e; git -C /vllm-workspace/vllm rev-parse HEAD; git -C /vllm-workspace/vllm-ascend rev-parse HEAD; git -C /vllm-workspace/Mooncake rev-parse HEAD; cd /vllm-workspace/vllm-ascend; find vllm_ascend -type f \( -path "*/envs.py" -o -path "*/platform.py" -o -path "*/perf_metrics.py" -o -path "*/kv_transfer.py" -o -path "*/pool_worker.py" -o -path "*/mooncake_backend.py" \) -print0 | sort -z | xargs -0 sha256sum'
record "assert embedded source identity" "${script_dir}/embedded-source-identity-assert.log" \
  python3 -c '
import sys
import hashlib
from pathlib import Path
...
assert actual[:3] == expected, actual
manifest = "".join(f"{line}\n" for line in actual[3:])
assert hashlib.sha256(manifest.encode()).hexdigest() == "9a8d9ea0b560561b3093ab418eaf8f4a320362e8c21ad272b6168e9deebd897e", actual[3:]
print("embedded source identity: PASS")
' "${script_dir}/embedded-source-identity.txt"
