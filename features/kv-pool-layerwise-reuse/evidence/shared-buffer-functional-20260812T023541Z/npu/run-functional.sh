...
readonly expected_image=${EXPECTED_IMAGE:-docker.io/library/vllm-ascend:kv-pool-layerwise-main-54503ece-a2-57d3c214e-df3f74ed-20260811T145302Z}
readonly expected_manifest=${EXPECTED_MANIFEST:-sha256:f8592141757f7e9976898858863e12ccd051ac4a3fd6ade7591f78d9769517e3}
readonly expected_config=${EXPECTED_CONFIG:-sha256:ce20411d6043d3830be7601c654b2c9a1d41fb923395cad2ea2e7ba200ebbbbd}
readonly asset_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly script_dir=${OUTPUT_DIR:-${asset_dir}}
readonly remote_root=${REMOTE_ROOT:-/tmp/shared-buffer-functional-20260812T023541Z}
...
record "capture embedded source identity" "${script_dir}/embedded-source-identity.txt" \
  kubectl exec -n "${namespace}" "${prefill_pod}" -c prefill-engine -- bash -c \
  'set -e; git -C /vllm-workspace/vllm rev-parse HEAD; git -C /vllm-workspace/vllm-ascend rev-parse HEAD; git -C /vllm-workspace/Mooncake rev-parse HEAD; sha256sum /vllm-workspace/vllm-ascend/vllm_ascend/distributed/kv_transfer/kv_pool/ascend_store/pool_worker.py'
...
assert actual[3].split()[0] == "54e3198504a3745b21e172d8e66c4c7c217bbc7642498e3bb4cdbab557b8b6ea", actual[3]
